...
async def do_card_scan(query, photo_key: str, ctx):
    ...
    try:
        ...
        prompt_text = (
            ...
            "Step 1: Read the set code at the bottom left (e.g. POR, PRE, ACE, MEG, OBF).\n"
            "Step 2: Read the card number at the bottom (e.g. 031/088 — can exceed total for secret rares).\n"
            "Step 3: Read the Pokemon name at the top.\n"
            "Step 4: Match the set code: "
            ...
            "PRICE: <estimated AUD value or Unknown>"
        )
        ...
        if not result:
            log.error("Card scan empty response: %s", data)
            error_msg = data.get("error", {}).get("message", "No response from model.")
            await query.edit_message_text(f"Could not identify the card: {error_msg}\n\nTry a clearer photo with the bottom of the card visible.")
            return
