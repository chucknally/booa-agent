    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{BOOA_API}/soul.md") as r:
                if r.status == 200:
                    AGENT_IDENTITY["soul"] = await r.text()
                    log.info("SOUL.md loaded (%d chars)", len(AGENT_IDENTITY["soul"]))
            async with session.get(f"{BOOA_API}/identity.md") as r:
                if r.status == 200:
                    AGENT_IDENTITY["identity"] = await r.text()
                    log.info("IDENTITY.md loaded (%d chars)", len(AGENT_IDENTITY["identity"]))
    except Exception as e:
        log.error("Failed to load BOOA identity: %s", e)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lines = AGENT_IDENTITY.get("identity", "").split("\n")
    for line in lines:
